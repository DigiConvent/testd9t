...
func (r *IAMRepository) CreateUser(user *iam_domain.UserWrite) (*uuid.UUID, core.Status) {
	...
	result, err := r.DB.Exec("insert into users (id, first_name, last_name, email, date_of_birth) values (?, ?, ?, ?, ?)", uid.String(), user.FirstName, user.LastName, user.Email, user.DateOfBirth)
	if err != nil {
		return nil, *core.InternalError(err.Error())
	}
