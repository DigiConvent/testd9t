...
export function create_tree_using_children<T extends HasChildren>(leaf: T, data: T[]) : CustomNode<T> {
    const node: CustomNode<T> = {
        key: leaf.id,
        data: leaf,
        children: []
...
export const create_tree_from_permission_group_facades = (leaf: PermissionGroupFacade, data: PermissionGroupFacade[]) : CustomNode<PermissionGroupFacade> => {
